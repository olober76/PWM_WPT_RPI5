...
def submit_input():
    ...
    try:
        ...
        if frequency > 0  and frequency < 2000000:
            set_frequency(frequency)  # Call the dynamicHz module to change frequency
            output_entry.delete(0, END)
            output_entry.insert(0, f"{frequency}")
            print(f"Set to {frequency} Hz")
        else:
            set_frequency(frequency)
            output_entry.delete(0, END)
            output_entry.insert(0, f"Hardware limit")
            print(f"Set to 1000 Hz due to hardware limit")
